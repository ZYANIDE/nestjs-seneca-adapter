declare module '@nestjs/microservices' {
  ...
  export abstract class Server {
    // added versioningEnabled prop
    versioningEnabled: boolean;
    // added showApiVersion prop
    showApiVersion: boolean;
    // added versionExtractor prop
    versionExtractor?: <TRequest extends IGenericClientRequest<unknown>>(
      request: TRequest,
    ) => VersionValue;
    ...
    // added versions as one of the parameters of addHandler()
    addHandler(
      pattern: any,
    ...
    getOptionsProp<T extends MicroserviceOptions['options'], K extends keyof T>(
      obj: T,
      prop: K,
      defaultValue?: T[K],
    ): T[K];
    ...
    protected loadPackage<T = any>(
      name: string,
      ctx: string,
      loader?: Function, // eslint-disable-line
    ): T;
